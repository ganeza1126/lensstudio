...
for (var i = 0; i < framePieces.length; i++) {
    framePieces[i].getComponent("Component.MaterialMeshVisual").mainMaterial = frameMaterial;
}
...
function adjustForAspectRatio() {
    ...
    if (scaledWidth >= scaledHeight) {
        ...
        script.pictureImage.getTransform().setLocalPosition(new vec3(0.0, 0.5, 0.01));
...
function setPartShadowDensity(part) {
    var baseMeshVisuals = part.getComponents("Component.BaseMeshVisual");
    for (var i = 0; i < baseMeshVisuals; i++) {
        var mesh = baseMeshVisuals[i];
